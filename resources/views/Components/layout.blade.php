...
<div class="min-h-full">
        ...
        <div class="flex items-center">
          ...
          <div class="hidden md:block">
            <div class="ml-10 flex items-baseline space-x-4">
              ...
              <!-- Rule of Thumb for request()->is() in Laravel:
              request()->is('/')  // ✅ matches the homepage
              request()->is('about')      // ✅ matches /about
              request()->is('contact')    // ✅ matches /contact
              request()->is('blog/*')     // ✅ matches /blog/anything -->
        ...
        <div class="hidden md:block">
          <div class="ml-4 flex items-center md:ml-6">
            ...
            <form action="/logout" method="POST">
              @csrf
                <x-form-button :logout=true >Log Out</x-form-button>
            </form>
            @endauth
          </div>
        </div>
  ...
  <header class="relative bg-white shadow-sm">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 justify-between flex">
      <h1 class="text-3xl font-bold tracking-tight text-gray-900">{{$heading}}</h1>
      <x-button href="/jobs/create" class="bg-indigo-600 hover:bg-indigo-700">Create New Job<x-slot:span>+</x-slot:span></x-button>
    </div>
  </header>
  <main>
